...
def top_player_ids(info, statistics, formula, numplayers):
    ...
    for row in statistics:
        ...
        ans.append(tuple([row[info['playerid']], formula(info, row)]))

    ans.sort(reverse=True, key=lambda pair: pair[1])
    return ans[:numplayers]
...
def lookup_player_names(info, top_ids_and_stats):
    ...
    for pair in top_ids_and_stats:
        row = masterfile[pair[0]]
        line = '{:.3f} --- {} {}'.format(float(pair[1]), 
                                         row[info['firstname']],
                                         row[info['lastname']]
                                        )
        ans.append(line)
...
def compute_top_stats_year(info, formula, numplayers, year):
    ...
    battingfile = read_csv_as_list_dict(info['battingfile'], info['separator'], info['quote'])
    year_data = filter_by_year(battingfile, year, info['yearid'])
    top_player_id = top_player_ids(info, year_data, formula, numplayers)

    return lookup_player_names(info, top_player_id)
